...
const GFONT = `@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@400;600;700;800;900&family=Roboto:wght@300;400;500;700&family=Roboto+Mono:wght@400;500&family=Raleway:wght@300;400;600&family=Cormorant+Garamond:wght@400;500;600&display=swap');`;

const THEMES = {
  blue:   { h1: '#0d2b6e', h2: '#1749b1', acc: '#f0c040', bg: '#eef2fb' },
  green:  { h1: '#064e3b', h2: '#059669', acc: '#6ee7b7', bg: '#edfaf4' },
  maroon: { h1: '#6b0f0f', h2: '#b91c1c', acc: '#fca5a5', bg: '#fdf2f2' },
  navy:   { h1: '#0f172a', h2: '#1e3a5f', acc: '#93c5fd', bg: '#eff4fb' },
  purple: { h1: '#3b0764', h2: '#7c3aed', acc: '#ddd6fe', bg: '#f5f0ff' },
};
...
function buildCardFront(s, photo, logo, phone, year, prin, theme) {
  ...
  const photoEl = photo
    ...
    : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#e8edf5;font-size:44px;font-weight:900;color:#aab4cc;font-family:sans-serif;">${((s.fn||'?')[0]).toUpperCase()}</div>`;

  let qrEl = '';
  try {
    const qd = encodeURIComponent(`${s.fn||''} ${s.ln||''}\nClass:${s.cls||''} Roll:${s.roll||''}\nAdm:${s.admno||''}\nFather:${s.father||''}\nPh:${mob}`);
    qrEl = `<img src="https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${qd}&margin=3&ecc=M" width="135" height="135" style="display:block;" alt="QR"/>`;
  } catch(e){}

  const drow = (label, val) =>
    `<div style="margin-bottom:18px;"><div style="font-size:13px;color:#444;font-family:'Roboto',sans-serif;">${label}</div><div style="font-size:18px;font-weight:700;color:#111;font-family:'Montserrat',sans-serif;line-height:1.2;">${val}</div></div>`;

  const logoEl = logo
    ? `<img src="${logo}" style="width:86px;height:86px;object-fit:contain;display:block;">`
    : `<div style="width:86px;height:86px;display:flex;align-items:center;justify-content:center;font-size:48px;">🏫</div>`;

  return `<div style="width:${CW}px;height:${CH}px;border-radius:10px;overflow:hidden;box-shadow:0 16px 48px rgba(0,0,0,.4);display:flex;flex-direction:column;background:${th.bg};font-family:'Montserrat',sans-serif;">
<style>${GFONT} * { margin:0; padding:0; box-sizing:border-box; }</style>

<!-- Header: logo | divider | school name -->
<div style="background:#fff;padding:16px 16px;display:flex;align-items:center;gap:14px;border-bottom:3px solid ${th.h1};">
  ${logoEl}
  <div style="width:2px;height:72px;background:#ddd;flex-shrink:0;"></div>
  <div style="flex:1;">
    <div style="font-size:26px;font-weight:900;color:${th.h1};font-family:'Montserrat',sans-serif;text-transform:uppercase;line-height:1.05;letter-spacing:0.3px;">Lord Krishna</div>
    <div style="font-size:26px;font-weight:900;color:${th.h1};font-family:'Montserrat',sans-serif;text-transform:uppercase;line-height:1.05;letter-spacing:0.3px;margin-bottom:6px;">Public School</div>
    <div style="font-size:14px;color:${th.h1};font-family:'Cormorant Garamond',serif;font-weight:600;margin-bottom:1px;letter-spacing:0.4px;">Govt. Recognised</div>
    <div style="font-size:14px;color:${th.h1};font-family:'Cormorant Garamond',serif;font-weight:600;margin-bottom:1px;letter-spacing:0.4px;">Ishapur, Laxminagar, Mathura</div>
  </div>
</div>

<!-- "Student ID Card" bar -->
<div style="background:${th.bg};padding:8px 0;text-align:center;border-bottom:1px solid rgba(0,0,0,.08);">
  <span style="font-size:15px;font-weight:600;color:#333;font-family:'Montserrat',sans-serif;letter-spacing:1.5px;">Student ID Card</span>
</div>

<!-- Dark name bar -->
<div style="background:${th.h1};padding:10px 14px;text-align:center;">
  <span style="font-size:20px;font-weight:300;color:#fff;font-family:'Raleway',sans-serif;letter-spacing:2px;text-transform:uppercase;">${s.fn||'Student'} ${s.ln||''}</span>
</div>

<!-- Body -->
<div style="flex:1;display:flex;background:${th.bg};min-height:0;">
  <div style="width:165px;flex-shrink:0;padding:14px 10px 14px 14px;display:flex;flex-direction:column;align-items:flex-start;">
    <div style="width:155px;height:190px;overflow:hidden;border:3px solid ${th.h1};border-radius:6px;flex-shrink:0;box-shadow:0 4px 12px rgba(0,0,0,.2);">${photoEl}</div>
    <div style="flex:1;min-height:8px;"></div>
    ${qrEl}
  </div>
  <div style="flex:1;padding:16px 14px 14px;display:flex;flex-direction:column;min-width:0;">
    ...
    ${s.blood ? drow('Blood Group:', `<span style="color:#c0392b;">${s.blood}</span>`) : ''}
    <div style="margin-top:auto;text-align:right;padding-top:6px;">
      <div style="font-size:24px;font-style:italic;color:${th.h1};font-family:'Georgia',serif;">${prin||'__________'}</div>
      <div style="font-size:12px;color:#555;font-family:'Roboto',sans-serif;margin-top:3px;">Principal</div>
    </div>
  </div>
</div>
...
export function buildCardBack(s, logo, phone) {
  const mob = s.fphone || s.ph || '—';
  ...
  const infoRow = (label, val) =>
    `<div style="display:flex;align-items:baseline;gap:6px;margin-bottom:9px;">
      <div style="font-size:13px;color:#222;font-family:'Roboto',sans-serif;white-space:nowrap;">${label}</div>
      <div style="font-size:14px;font-weight:700;color:#000;font-family:'Montserrat',sans-serif;flex:1;line-height:1.4;">${val}</div>
    </div>`;
...
<div style="padding:16px 16px 12px;display:flex;flex-direction:column;">
  ${s.blood ? infoRow('Blood Group :', `<span style="font-size:15px;font-family:'Montserrat',sans-serif;color:#c0392b;">${s.blood}</span>`) : ''}
  ${s.ph ? infoRow('Alt. Contact No. :', s.ph) : ''}
  ${infoRow("Father's Name :", s.father||'—')}
  ${infoRow("Mother's Name :", s.mother||'—')}
  ${s.fphone ? infoRow("Father's Contact No :", s.fphone) : ''}
  <div style="margin-bottom:4px;font-size:13px;color:#222;font-family:'Roboto',sans-serif;">Address :</div>
  <div style="font-size:13px;font-weight:700;color:#000;font-family:'Montserrat',sans-serif;line-height:1.55;margin-bottom:14px;">${addr}</div>
  <div style="font-size:13px;font-weight:700;color:#000;text-decoration:underline;margin-bottom:7px;font-family:'Roboto',sans-serif;">Rules to be followed :</div>
  <div style="font-size:11.5px;color:#111;line-height:1.85;font-family:'Roboto',sans-serif;">
    <div style="margin-bottom:2px;">• This card must be carried by the student at all times within school premises.</div>
    <div style="margin-bottom:2px;">• If found, please return this card to the school office.</div>
    <div style="margin-bottom:2px;">• Loss of ID card must be reported to the class teacher immediately.</div>
    <div style="margin-bottom:2px;">• A fine will be charged for re-issuance of a lost card.</div>
    <div>• Misuse of this card is a punishable offence as per school discipline policy.</div>
  </div>
</div>
<div style="flex:1;background:#fff;"></div>
<div style="height:150px;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;flex-shrink:0;overflow:hidden;">
  ${logo
    ? `<img src="${logo}" style="height:100px;object-fit:contain;opacity:0.45;">`
    : `<div style="font-size:80px;opacity:0.35;line-height:1;">🏫</div>`}
  <div style="font-size:17px;font-weight:800;color:#ccc;font-family:'Montserrat',sans-serif;letter-spacing:1px;text-transform:uppercase;">Lord Krishna Public School</div>
</div>
<div style="background:#111;padding:14px 14px;text-align:center;flex-shrink:0;">
  <div style="font-size:12px;color:#ddd;font-family:'Roboto',sans-serif;line-height:1.7;">Ishapur, Laxminagar, Mathura - 281204 (UP) India</div>
  <div style="font-size:12px;color:#aaa;font-family:'Roboto Mono',monospace;margin-top:4px;">Phone: +91 9997360040, +91 8650616990</div>
</div>
</div>`;
}
...
export function printCard(s, photo, logo, phone, year, prin, theme) {
  ...
  const back = buildCardBack(s, logo, phone);
